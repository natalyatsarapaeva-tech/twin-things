...
test('filterItems: категория, тег, статус, локация, текст', () => {
  const items = [
    ...
    makeItem({ name: 'Диван', category: 'furniture', location: 'Гостиная', status: 'wishlist' }),
  ...
  assert.equal(filterItems(items, { status: 'wishlist' }).length, 1);
  ...
  assert.equal(filterItems(items, { status: 'have' }).length, 2);
});
...
test('statusLabel: известные статусы переводятся', () => {
  assert.equal(statusLabel('have'), 'В наличии');
  assert.equal(statusLabel('wishlist'), 'Вишлист');
